...
class TurnsDetector:
    ...
    def process_record(self, dataset, subject_id, record_id, output_file):
        ...
        # Find turns 
        turns, left_filter, right_filter = self.find_turns(cdg, events, left_events, right_events, output_file, subject_id, record_id)
    ...
    def find_turns(self, cdg, events, left_events, right_events, out_dir, subject, record):
        ...
        self.plot_charts(time, cdg_z, cdg_z_hat, peaks_times, peaks_vals, events, out_dir, subject + "_" + record)
    ...
    def run(self):
        ...
        for subject in self.records.keys():
            logger("[Turns Detector] Process " + subject, msgType=LoggerType.OKBLUE)
            for record_id in self.records[subject]:
                logger("[Turns Detector] Record " + record_id, msgType=LoggerType.OKBLUE)
                turns, left_filter, right_filter = self.process_record(self.dataset, subject, record_id, self.output)
                self.turn_list.append((subject, record_id, turns, left_filter, right_filter))
        
        turns_table = pd.DataFrame(self.turn_list, columns=["Subject", "Record", "Turns_Count", "Turns_Left_Strides_Filtering", "Turns_Right_Strides_Filtering"])
        turns_table.to_excel(self.output, index=False)
        logger("[Turns Detector] Errors count " + str(len(self.errors)), msgType=LoggerType.OKBLUE)
